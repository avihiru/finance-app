 ...
 * 2. Add Web app → copy firebaseConfig values below
 * 3. Firestore Database → Create database → Start in test mode
 * 4. Firestore → Rules → paste permanent rule (see FIREBASE_SETUP.md)
 * ────────────────────────────────────────────────────────────────
 */
...
var FinanceDB = (function () {
  ...
  // Returns ISO-like datetime string in LOCAL timezone (fixes UTC offset date shift)
  function nowISO() {
    var d = new Date();
  ...
  function seedIfEmpty() {
    return init().then(function () {
      ...
      if (!snaps[0].empty || !snaps[1].empty) return;
      ...
      var batch = _db.batch();
      txSeed.forEach(function (r) { batch.set(_db.collection('transactions').doc(), r); });
      savingSeed.forEach(function (r) { batch.set(_db.collection('saving').doc(), r); });
      return batch.commit();
    });
  }
    ...
    return init().then(function () {
      ...
      return {
        ...
        transactions:   rows.map(function (r) {
          return {
            id:          r.id,          // Firestore string ID — kept as-is
            type:        r.type,
            category:    r.category,
            amount:      r.amount,
  ...
  // ── FIX: id is a Firestore string — no Number() cast ────────────
  ...
  function getSavingData() {
    return init().then(function () {
      ...
      ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
        .forEach(function (a) { if (monthly[a]) ordered[a] = monthly[a]; });
